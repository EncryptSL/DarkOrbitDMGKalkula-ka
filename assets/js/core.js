document.getElementById("calc").onclick = function () {
    ...
    var dmg_without_resources =+ calculation_dmg(laser_typ, lasers_ship, ship_designs,lasers_drones, lasers_drones_designs, "WITHOUT_RESOURCES")
 ...
 * @return number
 */
function select_lasers_dmg(laser_typ) {
    const lasers = [
        {name: "LF_3", dmg: 150},
        {name: "LF_4", dmg: 200},
        {name: "PROMETHEUS", dmg: 210 + 200}
    ]
    return lasers.filter(subject => subject.name === laser_typ)
        .map((e) => {
            return e.dmg;
    })
}
 ...
 * @return number
 */
function select_ship_design(ship_designs) {
    ...
    return data.filter(subject => subject.name === ship_designs)
        .map((e) => {
            return e.dmg;
    })
}
 ...
 * @return number
 */
function select_drones_designs(lasers_drones_designs) {
    if (lasers_drones_designs === "NOTHING")
        return 0
    else if (lasers_drones_designs === "HERCULES")
        return 0
    else if (lasers_drones_designs === "HAVOC")
        return 10
}
 ...
 * @return number
 */
function calculation_of_upgrades(laser_typ, upgrades) {
    if (upgrades === "NO")
        return 0
 ...
 * @return number
 */
function calculation_dmg(laser_typ, lasers_ship, ship_designs, lasers_drones, lasers_drones_designs, resource, type) {
    if (lasers_drones > 20)
        return 20

    var dmg_from_ship = + select_lasers_dmg(laser_typ) * lasers_ship + percentage(select_lasers_dmg(laser_typ) * lasers_ship, select_ship_design(ship_designs))
    var dmg_from_drones = + select_lasers_dmg(laser_typ) * lasers_drones + percentage(select_lasers_dmg(laser_typ) * lasers_drones, select_drones_designs(lasers_drones_designs))

    var total = dmg_from_ship + dmg_from_drones + percentage(dmg_from_ship + dmg_from_drones, select_resources(resource))

    if (type === "STANDARD") {
        total = select_lasers_dmg(laser_typ) * lasers_ship + select_lasers_dmg(laser_typ) * lasers_drones
    } else if (type === "WITHOUT_RESOURCES") {
        total = dmg_from_ship + dmg_from_drones
    }
